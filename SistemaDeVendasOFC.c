...
#define MAX_PRODUTOS 100#include <curses.h>
#include <string.h>
#include <stdlib.h>
#include <ctype.h>
#include <locale.h>
...
void realizar_venda() {
    ...
    for (i = 0; i < qtd_compras; i++) {
        achou = 0;
        mvwprintw(rea_ven, 7 + i * 2, 10, "Nome do %d - produto: ", i + 1);
        echo();
        wgetnstr(rea_ven, nome_pro, sizeof(nome_pro) - 1);
        noecho();
        ...
        for (x = 0; x < numProdutos; x++) {
            ...
            if (strcmp(compara, nome_pro) == 0) {
                ...
                if (produtos[x].quantidade == 0) {
                    wattron(rea_ven, A_BOLD | COLOR_PAIR(2));
                    mvwprintw(rea_ven, 10 + i * 2, 10, "Nao ha quantidade deste produto no estoque...");
                    wattroff(rea_ven, A_BOLD | COLOR_PAIR(2));
                    wrefresh(rea_ven);
                    wgetch(rea_ven);
                    delwin(rea_ven);
                ...
                do {
                    wattron(rea_ven, A_BOLD | COLOR_PAIR(4));
                    mvwprintw(rea_ven, 9 + i * 2, 10, "Quantidade de '%s' em estoque: %d", produtos[x].nome, produtos[x].quantidade);
                    wattroff(rea_ven, A_BOLD | COLOR_PAIR(4));
                    mvwprintw(rea_ven, 10 + i * 2, 10, "Quantidade de compra do %d - produto: ", i + 1);
                    echo();
                    wgetnstr(rea_ven, qtd, sizeof(qtd) - 1);
                    noecho();
                    quantidade = atoi(qtd);
                ...
                wattron(rea_ven, A_BOLD | COLOR_PAIR(4));
                mvwprintw(rea_ven, 12 + i * 2, (width - strlen("Compra realizada com sucesso!!")) / 2, "Compra realizada com sucesso!!");
                wattroff(rea_ven, A_BOLD | COLOR_PAIR(4));
                break;
            }
        }
